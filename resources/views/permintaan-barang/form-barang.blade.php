					...
					<form class="form-horizontal form-bordered" action="/tambah-kebutuhan-barang" method="POST" >
										...
										<div class="panel-body">
											<div class="form-group">
													...
													<div class="col-md-6">
														<select data-plugin-selectTwo class="form-control populate" name="kode_jenis" required>
														<option selected disabled value="">Pilih Barang</option>  
															@foreach ($jenis_barang as $item)
																<option value="{{ $item->kode_jenis }}">{{$item->nama}}</option>
															@endforeach
															
														</select>
														<small style="color:red">Tambah barang yang diminta sebelum submit</small> 
													</div>
											</div>
														...
														<div class="col-md-7">
															...
															@foreach ($id_terakhir as $item)
																...
																<a href="/permintaan-barang-user">
																	<button type="button" class="mb-xs mt-xs mr-xs btn btn-success"  onclick="return confirm('Apakah barang selesai ditambahkan?')">Simpan</button>
																</a>
											</div>
										</div>					
                    </form>
